...
void MotorController::corner_to_position(std::vector<int> corner_ticks) {
  this->lx16a->set_motor_mode(SERVO_LEFT_FRONT, corner_ticks[0]);
  this->lx16a->set_motor_mode(SERVO_RIGHT_FRONT, corner_ticks[1]);
  this->lx16a->set_motor_mode(SERVO_LEFT_BACK, corner_ticks[2]);
  this->lx16a->set_motor_mode(SERVO_RIGHT_BACK, corner_ticks[3]);
}
