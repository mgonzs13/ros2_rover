...
LX16A::LX16A(std::string serial_port, unsigned int baudrate) {
  this->serial = std::make_unique<Serial>(Serial(serial_port, baudrate));
...
ServoInfo LX16A::wait_for_response(uint8_t servo_id, uint8_t command) {
  while (true) {
    std::vector<uint8_t> data;
    data.reserve(10);
    this->serial->receive(data);
    ...
    // header part 2
    if (data.at(2) != SERVO_FRAME_HEADER) {
      continue;
    }
    ...
    for (uint8_t i = 5; i < length - 3 + 5; i++) {
      result.params.push_back(data.at(i));
    }
...
ServoInfo LX16A::query(uint8_t servo_id, uint8_t command) {
  const std::lock_guard<std::mutex> lock(*this->mtx);
...
uint8_t LX16A::word(int low, int high) { return int(low) + int(high) * 256; }
...
uint8_t LX16A::get_servo_id(uint8_t servo_id) {
  ServoInfo response = this->query(servo_id, SERVO_ID_READ);
...
std::vector<uint8_t> LX16A::get_prepared_move(uint8_t servo_id) {
  ServoInfo response = this->query(servo_id, SERVO_MOVE_TIME_WAIT_READ);

  return {this->word(response.params[0], response.params[1]),
          this->word(response.params[2], response.params[3])};
}

// position functions
int LX16A::get_position_offset(uint8_t servo_id) {
  ServoInfo response = this->query(servo_id, SERVO_ANGLE_OFFSET_READ);
  int deviation = response.params[0];
...
std::vector<uint8_t> LX16A::get_position_limits(int8_t servo_id) {
  ServoInfo response = this->query(servo_id, SERVO_ANGLE_LIMIT_READ);
...
int LX16A::get_position(uint8_t servo_id) {
  ServoInfo response = this->query(servo_id, SERVO_POS_READ);
  int position = this->word(response.params[0], response.params[1]);
...
std::vector<uint8_t> LX16A::get_voltage_limits(uint8_t servo_id) {
  ServoInfo response = this->query(servo_id, SERVO_VIN_LIMIT_READ);
...
uint8_t LX16A::get_voltage(uint8_t servo_id) {
  ServoInfo response = this->query(servo_id, SERVO_VIN_READ);

  return word(response.params[0], response.params[1]);
}
...
uint8_t LX16A::get_max_temperature_limit(uint8_t servo_id) {
  ServoInfo response = this->query(servo_id, SERVO_TEMP_MAX_LIMIT_READ);
  return response.params[0];
}

uint8_t LX16A::get_temperature(uint8_t servo_id) {
  ServoInfo response = this->query(servo_id, SERVO_TEMP_READ);
  return response.params[0];
}
...
uint8_t LX16A::get_mode(uint8_t servo_id) {
  ServoInfo response = this->query(servo_id, SERVO_OR_MOTOR_MODE_READ);
  return response.params[0];
}
...
bool LX16A::is_motor_on(uint8_t servo_id) {
  ServoInfo response = this->query(servo_id, SERVO_LOAD_OR_UNLOAD_READ);
  return response.params[0] == 1;
}
...
int LX16A::get_motor_speed(uint8_t servo_id) {
  ...
  if (response.params[0] != 1)
    return 0;

  int speed = this->word(response.params[2], response.params[3]);
...
bool LX16A::is_led_on(uint8_t servo_id) {
  ServoInfo response = this->query(servo_id, SERVO_LED_CTRL_READ);
  return response.params[0] == 0;
}
...
uint8_t LX16A::get_led_errors(uint8_t servo_id) {
  ServoInfo response = this->query(servo_id, SERVO_LED_ERROR_READ);
  return response.params[0];
}
